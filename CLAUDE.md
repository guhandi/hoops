...
- Email carries a slim summary body (CID-inline strip.png) + one session-zip attachment (every session file; open `report.html` inside for the interactive report — audio-synced movie replay, impact-aligned replay with 🤥 no-contact flags (`impacts.json` sidecar from `src/hoops/impacts.py`, ffmpeg-optional post-processing), waveform scrubber, runs/chase annotations, shot-anchored transcript, SVG charts, `src/hoops/report_html.py`); `narrative.json` persisted per session.
...
7. Owner rewires the phone Shortcut to POST to the cloud endpoint (`docs/shortcut-setup.md`), records a real test session end-to-end, then unloads the launchd plist (kept on disk for rollback) and clears stray recordings from the iCloud inbox.
